...
#                    })
a=collection_1.aggregate([{"$match":{'activateDate':{'$gte':datetime.strptime('20161014','%Y%m%d'),#改这个时间就行了
                                                     '$lt':datetime.strptime('20161015','%Y%m%d')},
                                        'os': 'ios'}
                           },
                          {"$project":{'device':1}}
                          ])
...
for i in a:
    ...
    number2=number2+1
    print(number2)
b= collection_1.aggregate([{"$match": {'activateDate': {'$gte': datetime.strptime('20161014', '%Y%m%d'),#改这个时间
                                                         '$lt': datetime.strptime('20161015', '%Y%m%d')},
                                        'os': 'android'}
                             },
                            {
...
for i in b:
    ...
    number3=number3+1
    print(number3)
c=collection_2.aggregate([{"$match":{'serverTime':{'$gte':datetime.strptime('20161014','%Y%m%d'),
                                                   '$lt':datetime.strptime('20161021','%Y%m%d')
                                                  },
                                    'os':'ios','device':{'$in': e},
...
d = collection_2.aggregate([{"$match": {'serverTime': {'$gte': datetime.strptime('20161014', '%Y%m%d'),
                                                       '$lt': datetime.strptime('20161021', '%Y%m%d')
                                                       },
